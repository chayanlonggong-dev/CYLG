...
export default function EditModelModal({
  ...
  useEffect(() => {
    ...
    setForm({
      ...
      introductionZhTW: model.introductionZhTW ?? "",
      introductionZhCN: model.introductionZhCN ?? "",
      introductionJa: model.introductionJa ?? "",
      introductionKo: model.introductionKo ?? "",
    });
  ...
  function updateField<K extends keyof AdminModel>(
    key: K,
    value: AdminModel[K]
  ) {
    setForm((current) => ({
  ...
  async function handleSave() {
    ...
    try {
      ...
      const payload = {
        ...form,
        code: form.level + String(form.number).padStart(3, "0"),
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/80 px-4 py-6 backdrop-blur-sm">
      <div className="relative max-h-[95vh] w-full max-w-6xl overflow-y-auto rounded-3xl border border-yellow-500/20 bg-[#101010] p-8 shadow-2xl">
        <button
          type="button"
          onClick={onClose}
          className="absolute right-5 top-5 rounded-full border border-gray-600 px-4 py-2 text-white hover:border-red-500 hover:text-red-400"
        >
          ✕
        </button>
        ...
        <div className="max-w-5xl">
          ...
          <div className="mt-8 grid gap-6 md:grid-cols-2">
            ...
            <div>
              ...
              <input
                ...
                value={form.number}
                onChange={(event) =>
                  updateField("number", Number(event.target.value))
                }
                className="w-full rounded-2xl border border-yellow-500/20 bg-[#181818] p-4 text-white"
              />
            </div>
            ...
            <div>
              ...
              <input
                ...
                disabled
                className="w-full rounded-2xl border border-yellow-500/20 bg-[#181818] p-4 text-gray-400"
              />
            </div>
          ...
          <div className="mt-8 flex flex-wrap gap-4">
            <label className="flex items-center gap-3 rounded-2xl border border-yellow-500/20 bg-[#181818] px-4 py-3 text-sm text-white">
              <input
                ...
                checked={form.online}
                onChange={(event) =>
                  updateField("online", event.target.checked)
                }
              />
              Online
            </label>
          ...
          <div className="mt-8">
            <IntroductionEditor
              value={form.introductionEn}
              onChange={(value) =>
                updateField("introductionEn", value)
              }
              introductionZhTW={form.introductionZhTW}
              introductionZhCN={form.introductionZhCN}
              introductionJa={form.introductionJa}
              introductionKo={form.introductionKo}
              onTranslationChange={(language, value) => {
                if (language === "zhTW")
                  updateField("introductionZhTW", value);
                if (language === "zhCN")
                  updateField("introductionZhCN", value);
                if (language === "ja")
                  updateField("introductionJa", value);
                if (language === "ko")
                  updateField("introductionKo", value);
              }}
            />
          </div>
