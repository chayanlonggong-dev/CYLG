...
function buildContactMessage( 
  ...
  const interestedInModel = contact.interestedInModel 
  ...
  const interestedInServices = contact.interestedInServices || "I am interested in your services."; 
 
  const requestInfo = contact.requestInfo || "Please let me know the details."; 
 
  const thankYou = contact.thankYou || "Thank you."; 
 
  // 统一改成一整段（无任何换行符号），不管有无 modelId 
  // 所有语言（简体/繁体/日语/韩语）也改成一行连贯的句子 
  return `${greeting} ${interestedInModel} ${interestedInServices} ${requestInfo} ${thankYou}`; 
} 
      ...
      > 
        ...
        > 
          ...
          <button 
            ...
            className="absolute right-5 top-5 text-3xl text-gray-400 hover:text-white" 
          ...
          <h2 className="mt-2 text-center text-3xl font-bold text-yellow-500"> 
...
className="w-full rounded-full border px-6 py-4 text-center transition-all hover:text-white" 
